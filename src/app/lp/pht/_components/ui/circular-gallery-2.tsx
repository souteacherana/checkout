/* eslint-disable @typescript-eslint/no-explicit-any -- componente de UI portado (WebGL/animação); tipagem original preservada */
"use client";
...
class Media {
  ...
  onResize(
    ...
    const baseHeightMultiplier = isMobile ? 750 : 1150;
    const baseWidthMultiplier = isMobile ? 580 : 900;

    this.scale = this.screen.height / 1500;
    this.plane.scale.y =
      (this.viewport.height * (baseHeightMultiplier * this.scale)) / this.screen.height;
    this.plane.scale.x =
      (this.viewport.width * (baseWidthMultiplier * this.scale)) / this.screen.width;
    this.program.uniforms.uPlaneSizes.value = [
      this.plane.scale.x,
      this.plane.scale.y,
    ];
    this.padding = isMobile ? 1.0 : 1.5;
...
class App {
  ...
  scrollSpeed: number;
  // `position` guarda onde o arrasto começou (usado em onTouchDown/Move)
  ...
  viewport!: { width: number; height: number };
  raf!: number;
  // Atribuídas em addEventListeners(), chamado pelo constructor — mesmo
  // padrão de definite assignment já usado nas propriedades acima.
  boundOnResize!: () => void;
  ...
  addEventListeners() {
    ...
    // "mousewheel" é o evento legado (Safari antigo) e não existe na tipagem
    // padrão; o cast mantém o fallback sem afrouxar o resto do arquivo.
